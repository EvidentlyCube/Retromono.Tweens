...
namespace Retromono.Tweens {
	...
	public class TweenAnimateDouble : TweenAnimateBase {
        ...
        public TweenAnimateDouble(TimeSpan duration, double from, double to, Action<double> updateCallback, Func<double, double> easingFunction = null, Action finishedCallback = null) 
            : base(duration, easingFunction, finishedCallback) {
            _from = from;
            _to = to;
            _updateCallback = updateCallback ?? throw new ArgumentNullException(nameof(updateCallback), "Update callback cannot be null");
        }
